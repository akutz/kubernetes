		...
		case "active":
			if r.TryDecodeAsNil() {
				...
				if false {
					...
					h.decSlicev1_ObjectReference((*[]pkg2_v1.ObjectReference)(yyv379), d)
				}
			}
...
func (x *ScheduledJobStatus) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		if false {
			...
			h.decSlicev1_ObjectReference((*[]pkg2_v1.ObjectReference)(yyv384), d)
		}
	}
	yyj383++
	if yyhl383 {
...
func (x codecSelfer1234) decSliceJob(v *[]Job, d *codec1978.Decoder) {
	...
	if yyl390 == 0 {
		...
		if yyl390 > cap(yyv390) {
			...
			yyv2390 := yyv390
			yyrl390, yyrt390 = z.DecInferLen(yyl390, z.DecBasicHandle().MaxInitLen, 824)
			if yyrt390 {
...
func (x codecSelfer1234) decSliceScheduledJob(v *[]ScheduledJob, d *codec1978.Decoder) {
	...
	if yyl402 == 0 {
		...
		if yyl402 > cap(yyv402) {
			...
			yyv2402 := yyv402
			yyrl402, yyrt402 = z.DecInferLen(yyl402, z.DecBasicHandle().MaxInitLen, 1072)
			if yyrt402 {
...
func (x codecSelfer1234) decSlicev1_ObjectReference(v *[]pkg2_v1.ObjectReference, d *codec1978.Decoder) {
	...
	if yyl409 == 0 {
		if yyv409 == nil {
			yyv409 = []pkg2_v1.ObjectReference{}
			yyc409 = true
		...
		if yyl409 > cap(yyv409) {
			...
			if yyrt409 {
				if yyrl409 <= cap(yyv409) {
					...
					yyv409 = make([]pkg2_v1.ObjectReference, yyrl409)
				}
			} else {
				yyv409 = make([]pkg2_v1.ObjectReference, yyrl409)
			}
		...
		for ; yyj409 < yyrr409; yyj409++ {
			...
			if r.TryDecodeAsNil() {
				yyv409[yyj409] = pkg2_v1.ObjectReference{}
			} else {
		...
		if yyrt409 {
			for ; yyj409 < yyl409; yyj409++ {
				yyv409 = append(yyv409, pkg2_v1.ObjectReference{})
				yyh409.ElemContainerState(yyj409)
				if r.TryDecodeAsNil() {
					yyv409[yyj409] = pkg2_v1.ObjectReference{}
				} else {
		...
		for ; !r.CheckBreak(); yyj409++ {
			...
			if yyj409 >= len(yyv409) {
				yyv409 = append(yyv409, pkg2_v1.ObjectReference{}) // var yyz409 pkg2_v1.ObjectReference
				yyc409 = true
			}
			yyh409.ElemContainerState(yyj409)
			if yyj409 < len(yyv409) {
