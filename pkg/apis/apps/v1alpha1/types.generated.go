		...
		case "items":
			if r.TryDecodeAsNil() {
				...
				if false {
					...
					h.decSliceStatefulSet((*[]StatefulSet)(yyv122), d)
				}
			}
		default:
...
func (x *StatefulSetList) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		if false {
			...
			h.decSliceStatefulSet((*[]StatefulSet)(yyv129), d)
		}
	}
	for {
		yyj124++
	...
	if yyl143 == 0 {
		...
		if yyl143 > cap(yyv143) {
			...
			if yyrt143 {
				if yyrl143 <= cap(yyv143) {
					...
					yyv143 = make([]StatefulSet, yyrl143)
				}
			} else {
				yyv143 = make([]StatefulSet, yyrl143)
			}
		...
		if yyrt143 {
			for ; yyj143 < yyl143; yyj143++ {
				yyv143 = append(yyv143, StatefulSet{})
				yyh143.ElemContainerState(yyj143)
				if r.TryDecodeAsNil() {
					yyv143[yyj143] = StatefulSet{}
				} else {
		...
		for ; !r.CheckBreak(); yyj143++ {
			...
			if yyj143 >= len(yyv143) {
				yyv143 = append(yyv143, StatefulSet{}) // var yyz143 StatefulSet
				yyc143 = true
			}
			yyh143.ElemContainerState(yyj143)
			if yyj143 < len(yyv143) {
