...
func (c *lsHttpClient) init() error {
	...
	lsxFileName := path.Join(lsxPath, lsxName)
...
func (c *lsHttpClient) exec(cmdStr string, args ...string) (string, error) {
	...
	if err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			...
			return "", fmt.Errorf("libStorage executor exit code", errCode)
		} else {
			return "", fmt.Errorf("exec %v failed: %v", cmd, err)
		}
	}
...
func (c *lsHttpClient) dlExec() error {
	...
	_, err = os.Stat(lsxPath)
	if err != nil {
		if os.IsNotExist(err) {
			os.Mkdir(lsxPath, 0744)
		} else {
			return err
		}
	}

	lsxFile := path.Join(lsxPath, lsxName)
...
func (c *lsHttpClient) AttachVolume(id string) (string, error) {
	...
	device, err := c.waitForAttachedDevice(attach.Token)
	if err != nil {
		return "", err
	}
	return device, nil
...
func (c *lsHttpClient) DetachVolume(name string) error {
	if name == "" {
	...
	if detachedVol.ID != vol.ID {
		return fmt.Errorf("unexpected id for detached volume")
	}
...
func (c *lsHttpClient) DeleteVolume(name string) error {
	if name == "" {
		return fmt.Errorf("volume name missing")
	}
	if err := c.init(); err != nil {
...
func (c *lsHttpClient) waitForAttachedDevice(token string) (string, error) {
	...
	return "", fmt.Errorf("failed to get device for volume")
...
func (c *lsHttpClient) mapDevs(devs string) (map[string]string, error) {
